class PointService
	def self.pull_points_from_external_networks
	 	Twitter.search("#JustAddGirls #17s", since_id:Haiku.max(:tweet_id)).statuses.each do |tweet|
	 		user = User.where(name:tweet.user.screen_name || User.create!(
	 													uid: tweet.user.id,
	 		...
	 		if tweet.retweet?
	 			the_haiku = Haiku.where(tweet_id:tweet.id) #TODO:make sure original tweet exists
	 			the_haiku.user.add_point({point_type: :retweet, value:5,haiku: the_haiku})
		 	else
